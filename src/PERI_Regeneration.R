...
#write.table(RegenDensity, "Text Output/RegenDensity.txt", sep="\t") 
#Did not need to export data to clean in Excel because all year and locationID combinations were already present in data set
...
ggsave(filename = "Seedling Density.png") # <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
...
#write.csv(SmSaplingDensity, "SmSaplingDensity.csv")

#LgSaplingDensity

#write.csv(LgSaplingDensity, "LgSaplingDensity.csv")

SaplingDensity <- read_csv("SaplingDensity.csv")

SaplingDensity

ggplot(SaplingDensity, aes(x = MeanSapling)) +
  geom_bar()
